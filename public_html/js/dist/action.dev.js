...
(function () {
  ...
  var month = today.getMonth() + 1; // 獲取月資訊（從0開始 範圍：0-11）
  ...
  $('document').ready(function () {
    ...
    var eventTemplate = Handlebars.compile(source); // load DB event data to web
    ...
    $.each(events, function (index, item) {
      ...
      $('#calendar').find('.day-block[data-day="' + item.day + '"]').find('.events').append(eventUI); // Position today:
    ...
    $('.day-block').dblclick(function (e) {
      ...
      panel.selectedEvent = $(e.currentTarget); // selectedDayBlock -> 用以紀錄被點擊的 .event 位於哪個.day - block
      ...
      var id = panel.selectedEvent.data('id'); // console.log(id);
    ...
    $(panel.el).on('click', 'button', function (e) {
      if ($(this).is('.create') || $(this).is('.update')) {
        ...
        if ($(this).is('.update')) {
          var action = "event/update.php"; //remove old event data
        ...
        $.post(action, eventData).done(function (data, textStatus, xhr) {
          /* insert the data to front web */
          var eventUI = eventTemplate(data); // 依照時間排序：
          ...
          panel.selectedDayBlock.find('.event').each(function (index, item) {
            ...
            var newEventStartTime = data.start_time.split(':'); // console.log(eventStartTime, newEventStartTime);
          ...
          if (panel.selectedDayBlock.find('[data-id="' + data.id + '"]').length == 0) {
            ...
            $(panel.selectedDayBlock).find('.events').append(eventUI); // panel.selectedDayBlock.removeClass('selected');
          }
      ...
      if ($(this).is('.delete')) {
        ...
        $.post("event/delete.php", {
          ...
          var result = confirm("Are you sure you want to delete \" ".concat(title, " \" ? \nThis action cannot be undone."));
